...
<div class="heart_img"><img src="images/images_heart.png" alt="heaert"></div>
            ...
            <section id="menubar">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a data-toggle="tab" href="#Historyupload">Newst Upload</a></li>
				    <li v-for="categori, index in categories"><a data-toggle="tab" :href="'#historyCat' +categori.id" @click="showhistoryResCat(categori.id)">{{ categori.name }}</a></li>
				</ul>
	
			</div>
                                							                       
                                    
									    <HeaderSecTwo></HeaderSecTwo>
...
           },///End,
showhistoryResCat(id){
  console.log(id);
   var vm = this
                 axios.get(`/showAllHistoryResCat/${id}`)
          .then(response => {
             vm.historyes = response.data
			 console.log(response)
            console.log(response.data)
           serverBus.$emit('serverSelectedHistory', response.data);
          })
          .catch(error => {
            console.log(error)
          })
