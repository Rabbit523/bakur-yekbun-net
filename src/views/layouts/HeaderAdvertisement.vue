...
<div class="heart_img"><img src="images/images_heart.png" alt="heaert"></div>
            ...
            <section id="menubar">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a data-toggle="tab" href="#Advertupload" id="forAdvertupload">Newst Upload</a></li>
				  <li v-for="categori, index in categories"><a data-toggle="tab" :href="'#advertismentsCat' +categori.id" @click="showAdvertismentResCat(categori.id)">{{ categori.name }}</a></li>
				</ul>
                            ...
                            <input @keyup="showAllAdvertismentSearch()" class="form-control search-bar col-xs-6" type="text" name="searchAdvertisment"  v-model="searchAdvertisment" placeholder="Search...">
                        </div>
...
export default {
  ...
  mounted() {
    //edd
	this.showAllAdvertismentCatbukr();
  },
  methods: {
  showAllAdvertismentCatbukr()
           {
                var vm = this
                 axios.get('/advertismentCategoriesBukar')
          .then(response => {
...
           },///End,
      showAdvertismentResCat(id){
  console.log(id);
   var vm = this
                 axios.get(`/showAllAdvertismentResCat/${id}`)
          .then(response => {
             vm.advertisments = response.data
			 console.log(response)
            console.log(response.data)
           serverBus.$emit('serverSelectedAdvertisment', response.data);
          })
          .catch(error => {
            console.log(error)
          })
 ...
 showAllAdvertismentSearch(){
  if(this.searchAdvertisment.length > 0){
   $('.nav-tabs li:nth-child(2) a').tab('show');
      var vm = this
                 axios.get(`/showAllAdvertismentSearch?searchAdvertisment=${this.searchAdvertisment}`)
          .then(response => {
             vm.allAdvertisment = response.data.data
           
			
			 console.log(response.data.data.data)
			  serverBus.$emit('serverSelectedAdvertisment', response.data.data.data);
