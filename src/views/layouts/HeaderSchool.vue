...
<div class="heart_img"><img src="images/images_heart.png" alt="heaert"></div>
            ...
            <section id="menubar">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a data-toggle="tab" href="#Schoolupload" id="forSchoolupload">Newst Upload</a></li>
				  <li v-for="categori, index in categories"><a data-toggle="tab" :href="'#schoolCat' +categori.id" @click="showSchoolResCat(categori.id)">{{ categori.name }}</a></li>
				</ul>
                            ...
                            <input @keyup="showAllSchoolSearch()" class="form-control search-bar col-xs-6" type="text" name="searchSchool"  v-model="searchSchool" placeholder="Search...">
                        </div>
...
           },///End,
  ...
  console.log(id);
   var vm = this
                 axios.get(`/showAllSchoolResCat/${id}`)
          .then(response => {
             vm.schools = response.data
			 console.log(response)
            console.log(response.data)
           serverBus.$emit('serverSelectedSchools', response.data);
          })
          .catch(error => {
            console.log(error)
          })
 ...
 showAllSchoolSearch(){
  if(this.searchSchool.length > 0){
   $('.nav-tabs li:nth-child(2) a').tab('show');
      var vm = this
                 axios.get(`/showAllSchoolSearch?searchSchool=${this.searchSchool}`)
          .then(response => {
             vm.allschools = response.data.data
           
			
			 console.log(response.data.data.data)
			  serverBus.$emit('serverSelectedSchools', response.data.data.data);
