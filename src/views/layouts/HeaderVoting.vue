...
<div class="heart_img"><img src="images/images_heart.png" alt="heaert"></div>
            ...
            <section id="menubar">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a data-toggle="tab" href="#Votingupload" id="forVotingupload">Newst Upload</a></li>
				  <li v-for="categori, index in categories"><a data-toggle="tab" :href="'#votingCat' +categori.id" @click="showVotingResCat(categori.id)">{{ categori.name }}</a></li>
				</ul>
                            ...
                            <input @keyup="showAllvotingSearch()" class="form-control search-bar col-xs-6" type="text" name="searchVoting"  v-model="searchVoting" placeholder="Search...">
                        </div>
...
           },///End,
  ...
  console.log(id);
   var vm = this
                 axios.get(`/showAllVotingResCat/${id}`)
          .then(response => {
             vm.votings = response.data
			 console.log(response)
            console.log(response.data)
           serverBus.$emit('serverSelectedVotings', response.data);
          })
          .catch(error => {
            console.log(error)
          })
 ...
 showAllvotingSearch(){
  if(this.searchVoting.length > 0){
   $('.nav-tabs li:nth-child(2) a').tab('show');
      var vm = this
                 axios.get(`/showAllVotingsSearch?searchVoting=${this.searchVoting}`)
          .then(response => {
             vm.allvotings = response.data.data
           
			
			 console.log(response.data.data.data)
			  serverBus.$emit('serverSelectedVotings', response.data.data.data);
