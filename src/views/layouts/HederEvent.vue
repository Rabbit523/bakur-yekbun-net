...
<div class="heart_img"><img src="images/images_heart.png" alt="heaert"></div>
            ...
            <section id="menubar">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a data-toggle="tab" href="#Eventupload" id="forEventupload">Newst Upload</a></li>
				 
				  <li v-for="categori, index in categories"><a data-toggle="tab" :href="'#eventCat' +categori.id" @click="showEventResCat(categori.id)">{{ categori.name }}</a></li>
                            ...
                            <input @keyup="showAllEvelnt()" class="form-control search-bar col-xs-6" type="text" name="searchEvent"  v-model="searchEvent" placeholder="Search...">
                        </div>
...
           },///End,
  ...
  console.log(id);
   var vm = this
                 axios.get(`/showAllEventResCat/${id}`)
          .then(response => {
             vm.events = response.data
			 console.log(response)
            console.log(response.data)
           serverBus.$emit('serverSelectedEvent', response.data);
          })
          .catch(error => {
            console.log(error)
          })
  ...
  if(this.searchEvent.length > 0){
   $('.nav-tabs li:nth-child(2) a').tab('show');
      var vm = this
                 axios.get(`/showAllEvelnt?searchEvent=${this.searchEvent}`)
          .then(response => {
             vm.allevents = response.data.data
           
			
			 console.log(response.data.data.data)
			  serverBus.$emit('serverSelectedEvent', response.data.data.data);
